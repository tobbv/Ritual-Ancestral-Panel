...
El seguimiento del pedido usa las etapas Recibido → Preparando → Listo → Entregado a transportadora → En camino → Entregado. Para retiro local se omiten las dos etapas de transporte. La migración `supabase-estado-transportadora.sql` permite al portal de delivery guardar el nuevo estado y mantiene la preparación en «Listo» al despachar; ya se aplicó al proyecto Supabase conectado el 22 de septiembre de 2026. Para instalar el panel en **otro** proyecto Supabase, ejecutá primero `supabase-delivery-setup.sql` y luego `supabase-estado-transportadora.sql`. Publicar el HTML en GitHub no ejecuta SQL.
